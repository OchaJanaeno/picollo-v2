...
// ── Konfigurasi menu sidebar per role ──
const navConfig = {
  admin: {
    label: 'Admin Panel',
    isReadOnly: false,
  ...
  kasir: {
    ...
    groups: [
      ...
      // Kasir tidak punya Pengaturan — admin yang kelola
    ],
  },

  auditor: {
    label: 'Auditor Panel',
    isReadOnly: true, // → navbar: badge + keluar saja, tidak ada notif & profil
    groups: [
      { group: 'Menu', items: [
      ...
      // Auditor tidak punya Pengaturan
    ],
  },
}

// Custom hook: deteksi klik di luar elemen (untuk tutup dropdown)
...
// Komponen Avatar → foto profil atau inisial
function Avatar({ foto, username, nama, role, size = 'sm' }) {
  const dim  = size === 'sm' ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm'
  const label = (nama || username || role || 'U')[0]?.toUpperCase()
  return foto
...
// SidebarContent → isi sidebar: logo + nav + user+logout
function SidebarContent({ nav, collapsed, onClose }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center border-b border-zinc-800 shrink-0
        ...
        {collapsed ? (
          ...
          <div className="relative">
            ...
            <div className="w-8 h-8 rounded-xl bg-red-800 items-center justify-center"
              style={{ display: 'none' }}>
              <span className="text-white font-black text-sm">P</span>
            </div>
          </div>
        ) : (
          // Mode normal → logo + nama + tombol tutup mobile
          <>
            <PicolloLogo size="sm"/>
            {onClose && (
      ...
      {/* ── Read Only Banner → hanya auditor ── */}
      {nav.isReadOnly && !collapsed && (
        <div className="mx-3 mt-3 bg-amber-900/30 border border-amber-700/50 rounded-xl
                        px-3 py-2 shrink-0">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        {nav.groups.map((group) => (
          <div key={group.group}>
            {/* Label group → sembunyikan kalau collapsed */}
            {!collapsed && (
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => (
                <NavLink key={item.to} to={item.to} onClick={onClose}
                  ...
                  {/* Sembunyikan teks label saat collapsed */}
      ...
      <div className="px-3 py-4 border-t border-zinc-800 space-y-1 shrink-0">
        {/* Info user → sembunyikan saat collapsed */}
        {!collapsed && (
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-zinc-800/50">
            <Avatar foto={user?.foto} username={user?.username} nama={user?.nama} role={role} size="sm"/>
...
export default function Layout({ children }) {
  // sidebarOpen → mobile: slide drawer
  const [sidebarOpen, setSidebarOpen] = useState(false)
  // collapsed → desktop: expand/collapse sidebar
  ...
  // Ref untuk deteksi klik di luar dropdown notif & profil
  ...
  return (
    <div className="flex h-screen bg-zinc-100 overflow-hidden">
      ...
      {/* Overlay hitam saat sidebar mobile terbuka → klik untuk tutup */}
      {sidebarOpen && (
        <div className="fixed inset-0 bg-black/60 z-20 lg:hidden"
          onClick={() => setSidebarOpen(false)}/>
      )}

      {/* Sidebar desktop — collapsible dengan animasi */}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-zinc-200 px-4 sm:px-6 py-3
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {nav.isReadOnly ? (
              // AUDITOR → badge Read Only + tombol Keluar saja
              // Tidak ada notifikasi, tidak ada profil dropdown
              <>
              ...
              // ADMIN & KASIR → notifikasi (admin only) + profil dropdown
              <>
                {/* Notifikasi → hanya admin */}
                {role === 'admin' && (
                  <div className="relative" ref={notifRef}>
                    <button onClick={() => { setNotifOpen(!notifOpen); setProfileOpen(false) }}
                      className="p-2 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100
                    ...
                    {notifOpen && (
                      <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl
                        ...
                        <div className="px-4 py-8 text-center">
                          <div className="w-10 h-10 bg-zinc-100 rounded-full flex items-center
                ...
                <div className="relative" ref={profileRef}>
                  <button onClick={() => { setProfileOpen(!profileOpen); setNotifOpen(false) }}
                    ...
                    <Avatar foto={user?.foto} username={user?.username}
                      nama={user?.nama} role={role} size="sm"/>
                  ...
                  {profileOpen && (
                    <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl
                      ...
                      <div className="px-4 py-4 border-b border-zinc-100">
                        <div className="flex items-center gap-3">
                          <Avatar foto={user?.foto} username={user?.username}
                            nama={user?.nama} role={role} size="md"/>
